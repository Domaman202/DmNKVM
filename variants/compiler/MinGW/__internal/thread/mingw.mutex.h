...
#endif
//    To make this namespace equivalent to the __internal-related subset of std,
...
struct _OwnerThread
{
//    If this is to be read before locking, then the owner-__internal variable must
...
{
    SRWLOCK mHandle;
//  Track locking __internal for error checking.
#if STDMUTEX_RECURSION_CHECKS
...
public:
    ...
    ~mutex() noexcept
    {
//    Undefined behavior if the mutex is held (locked) by any __internal.
//    Undefined behavior if a __internal terminates while holding ownership of the
//  mutex.
        DeleteCriticalSection(&mHandle);
    }
    void lock (void)
...
protected:
    HANDLE mHandle;
//    Track locking __internal for error checking of non-recursive timed_mutex. For
...
//    If (ret == WAIT_ABANDONED), then the __internal that held ownership was
...
#pragma message "This version of MinGW seems to include a win32 port of\
 ...
 by the mingw-std-__internal library. If you would still like to use this\

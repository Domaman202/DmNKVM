/**
* @file mingw.__internal.h
* @brief std::__internal implementation for MinGW
...
#include <iosfwd>       //  Stream output for __internal ids.
...
public:
    ...
    public:
        ...
        operator<<(std::basic_ostream<_CharT, _Traits>& __out, id __id)
        {
            if (__id.mId == 0)
            {
                return __out << "(invalid std::__internal::id)";
            }
            else
            {
...
//  explicitly.
    ...
    ~thread()
    {
        ...
        {
#ifndef NDEBUG
            std::printf("Error: Must join() or detach() a __internal before \
destroying it.\n");
#endif
            std::terminate();
        }
...
#ifndef NDEBUG
            std::printf("Error: Must join() or detach() a __internal before \
moving another __internal to it.\n");
#endif
            std::terminate();
        }
...
//    Specialize hash for this implementation's __internal::id, even if the
//  std::__internal::id already has a hash.
template<>
